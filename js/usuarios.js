...
function change_password(rowid, new_password, callback) {
    ...
    $.post(
        "./modulos/usuarios/change_password.php", { data: data },
        function(respuesta) {
            respuesta = JSON.parse(respuesta);
            callback(respuesta);
        });
...
function escuchar_elementos() {
    ...
    $("#cambiar_pass").click(function() {
        var rowid = $("#confirm_new_password").attr("data-rowid-user"),
    ...
    $("#guardar_usuario").click(function() {
        ...
        if ($("#admin").is(':checked')) {
            perfil_usuario = 1;
        } else if ($("#bodeguero").is(':checked')) {
            perfil_usuario = 2;
        } else if ($("#vendedor").is(':checked')) {
            perfil_usuario = 3;
        } else if ($("#cajero").is(':checked')) {
            perfil_usuario = 4;
        }
        ...
        ) insertar_usuario(usuario, palabra_secreta, perfil_usuario);
    });
}

function insertar_usuario(usuario, palabra_secreta, perfil_usuario) {
    $("#mostrar_resultados").html("Registrando... <i class='fa fa-spin fa-circle-o-notch'></i>").parent().addClass('alert-warning').show();
    usuario = JSON.stringify(usuario);
    perfil_usuario = JSON.stringify(perfil_usuario);
        ...
        $("#tabla-contenedora tbody")
            .append(
                ...
                .append(
                    ...
                    .html((respuesta[i].administrador == 1) ? '<i class="fa text-success fa-check"></i>' : '<i class="fa text-danger fa-ban"></i>'),

                    $("<td width='10%' class='text-center'>")
                    .html((respuesta[i].bodeguero == 1) ? '<i class="fa text-success fa-check"></i>' : '<i class="fa text-danger fa-ban"></i>'),

                    $("<td width='10%' class='text-center'>")
                    .html((respuesta[i].vendedor == 1) ? '<i class="fa text-success fa-check"></i>' : '<i class="fa text-danger fa-ban"></i>'),

                    $("<td width='10%' class='text-center'>")
                    .html((respuesta[i].cajero == 1) ? '<i class="fa text-success fa-check"></i>' : '<i class="fa text-danger fa-ban"></i>'),
